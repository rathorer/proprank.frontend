...
export default function () {
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        const newUserCreditials: NewUserCredentials = { email, password, name };
        const response = await createUser(newUserCreditials);
    ...
    return (
        <section className="">
            <div className=" items-center px-5 py-12 lg:px-20">
                <div className="flex flex-col w-full max-w-md p-10 mx-auto my-6 transition duration-500 ease-in-out transform bg-white rounded-lg md:mt-0 border border-gray">
                    <div className="mt-8">
                            ...
                            <form method="POST" className="space-y-6" onSubmit={handleSubmit}>
                                ...
                                <div className="space-y-1">
                                    ...
                                    <div className="mt-1 flex items-center relative">
                                        <input id="password" name="password" type={showPassword ? 'text' : "password"} autoComplete="current-password" required placeholder="Your Password" className="block w-full px-5 py-3 text-base text-neutral-600 placeholder-gray-300 transition duration-500 ease-in-out transform border border-gray rounded-lg bg-gray-50 focus:outline-none focus:border-transparent focus:ring-2 focus:ring-gray focus:ring-offset-2 " />
                                        <div className="absolute right-2" onClick={() => setShowPassword(!showPassword)}>
                                            {showPassword ? <img src="/eyeOff.png" alt="" /> : <img src="/eye.png" alt="" />}
                                        </div>
                                    </div>
                                </div>
                                <div>
